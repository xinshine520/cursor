...
| 技术层 | 技术栈 | 版本 | 说明 |
|--------|--------|------|------|
| 数据库 | PostgreSQL | 18.x | 最新稳定版，提供强大的关系型数据库能力 |
...
| description | 文本 | ❌ | 最多 10,000 字符 | null | 详细描述，支持富文本/Markdown |
| priority | 枚举 | ❌ | low/medium/high/critical | medium | 优先级 |
...
| title | VARCHAR(200) | NOT NULL | - | GIN/BTREE | Ticket 标题 |
| description | TEXT | NULLABLE | NULL | - | 详细描述 |
| priority | VARCHAR(10) | NOT NULL, CHECK | 'medium' | BTREE | 优先级：low/medium/high/critical |
...
COMMENT ON COLUMN tickets.description IS '详细描述，支持 Markdown';
COMMENT ON COLUMN tickets.priority IS '优先级：low/medium/high/critical';
...
| priority | string | ❌ | - | 优先级筛选：low / medium / high / critical |
...
```json
{
  "detail": [
    ...
    {
      "loc": ["body", "priority"],
      "msg": "value is not a valid enumeration member; permitted: 'low', 'medium', 'high', 'critical'",
...
**核心技术选型说明**:
- **React 19**: 使用最新的 React Compiler 和 Server Components（如需）
- **Vite 7**: 极速的开发服务器和构建工具
- **TanStack Query**: 服务器状态管理，自动缓存、重新验证
- **Zustand**: 客户端 UI 状态管理（可选，用于侧边栏展开等）
- **React Router**: 路由管理
- **Axios**: HTTP 客户端
- **Tailwind CSS 4**: 原子化样式，Lightning CSS 引擎
...
|--------|------|------|-------------|
| critical | 🔴 深红 | 🔥 | `text-red-600 bg-red-100` |
